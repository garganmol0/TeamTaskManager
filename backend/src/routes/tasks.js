...
router.post('/', protect, body('title').notEmpty(), body('project').notEmpty(), async (req, res, next) => {
  ...
  try {
    ...
    if (!project.members.includes(req.user._id)) return res.status(403).json({ message: 'Forbidden' })
    const task = await Task.create({ title: req.body.title, description: req.body.description, project: project._id, assignee: req.body.assignee, priority: req.body.priority, dueDate: req.body.dueDate })
...
router.put('/:id', protect, async (req, res, next) => {
  try {
    ...
    if (!project.members.includes(req.user._id) && req.user.role !== 'admin') return res.status(403).json({ message: 'Forbidden' })
